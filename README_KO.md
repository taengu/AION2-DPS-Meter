...
1. **Npcap** 설치:
   ...
   - **"Install Npcap in WinPcap API-compatible Mode"**를 반드시 체크해야 합니다.

2. 최신 릴리스를 다운로드하여 설치:
   👉 https://github.com/taengu/Aion2-Dps-Meter/releases

3. 바탕화면 바로가기 또는 시작 메뉴에서 **AION2 DPS Meter**를 실행합니다.

4. 앱을 처음 열 때 **Windows 방화벽** 허용 메시지가 나타납니다.
   - 메뉴를 확장하여 개인 및 공용 네트워크를 모두 체크하는 것이 좋습니다.
   - 이렇게 하면 데이터가 누락되지 않도록 할 수 있습니다.

5. 이전에 작동하던 미터가 작동을 멈춘 경우:
   - 새로고침 아이콘을 클릭합니다.
   - 그래도 작동하지 않으면 앱을 종료하고 다시 실행합니다.
...
JVM 빌드로 실행하면서 주기적인 메모리 사용량 스냅샷과 클래스 히스토그램을 캡처하려면:

```bash
./gradlew clean run
```

`run`은 이제 기본 개발 설정(30초 간격, 상위 50개 클래스)으로 프로파일러를 자동 활성화하고 로그를 `build/memory-profile/`에 기록합니다.

옵션:
- `--mem-profile`은 기본값(60초 간격, 상위 30개 클래스)으로 프로파일링을 활성화합니다.
- `--mem-profile-interval=<초>`는 스냅샷 주기를 변경합니다 (최소 5초).
- `--mem-profile-top=<개수>`는 각 히스토그램에 표시할 클래스 수를 제어합니다.
- `--mem-profile-output=<디렉토리>`는 로그 저장 위치를 변경합니다 (기본값: `memory-profile/`).

Gradle `run`의 경우 JVM 속성을 통해 기본값이 설정됩니다:
...
CLI 플래그나 자체 JVM 속성으로 이를 재정의할 수 있습니다.
각 실행마다 타임스탬프가 포함된 로그 파일이 기록되므로(기본값: `memory-profile/`; Gradle `run`: `build/memory-profile/`) 시간 경과에 따른 RAM 소비 현황을 파악할 수 있습니다.
...
**Q: 채팅 또는 명령어 기능이 지원되나요?**
- 현재는 지원되지 않습니다.

**Q: 적중 횟수가 스킬 시전 횟수보다 많습니다.**
- 다중 타격 스킬은 각 타격을 개별적으로 계산합니다.

...
//        if (packetLengthInfo.value < 32) return
    ...
    private fun parsingDamage(packet: ByteArray): Boolean {
        ...
        val targetInfo = reader.readVarIntOutput()
        if (targetInfo.length < 0) return logUnparsedDamage()
        if (reader.offset >= packet.size) return logUnparsedDamage()

        val switchInfo = reader.readVarIntOutput()
        if (switchInfo.length < 0) return logUnparsedDamage()
        if (reader.offset >= packet.size) return logUnparsedDamage()

        val flagInfo = reader.readVarIntOutput()
        if (flagInfo.length < 0) return logUnparsedDamage()
        if (reader.offset >= packet.size) return logUnparsedDamage()
        ...
        val temp = reader.offset

        val rawSkillCode = parseUInt32le(packet, reader.offset)
        val skillCode = normalizeSkillId(rawSkillCode)

        reader.offset = temp + 5

        val typeInfo = reader.readVarIntOutput()
        if (typeInfo.length < 0) return logUnparsedDamage()
        if (reader.offset >= packet.size) return logUnparsedDamage()
        ...
        val hits: List<Long>?
        val finalDamage: Int?
        val isMultiHit: Boolean
        val unknownInfo: VarIntOutput?
        if (isMultiHitDamageType(typeInfo.value)) {
            unknownInfo = null
            finalDamage = reader.readVarInt()
            if (finalDamage < 0) return logUnparsedDamage()
            val hitCount = reader.readVarInt()
            if (hitCount < 0) return logUnparsedDamage()
            var multiHit = hitCount > 1 && reader.remainingBytes() >= hitCount
            if (hitCount > 8) {
                multiHit = false
            }
            isMultiHit = multiHit
            hits = if (isMultiHit) {
                ...
                repeat(hitCount) {
                    val hitValue = reader.readVarInt()
                    if (hitValue < 0) return logUnparsedDamage()
            ...
            if (isMultiHit && hits.isEmpty()) return logUnparsedDamage()
        } else {
...
//        }
        ...
        if (isMultiHit) {
            ...
            pdp.setSpecials(specials)
            unknownInfo?.let { pdp.setUnknown(it) }
            pdp.setDamage(VarIntOutput(finalDamage ?: 0, 1))
            pdp.setLoop(loopInfo)
